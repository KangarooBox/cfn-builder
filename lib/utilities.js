...
exports.isSafeToApplyChangeSet = function(changeSet){
  var isSafe = _.reduce(changeSet.Changes, function(result, value){
...
// Are we in the correct AWS account for these blueprints?
exports.isCorrectAccount = async function(){
  const impliedAccountId = exports.GLOBALS.AccountId;
  var   myAccountId = null;
  var sts = new AWS.STS();

  var promise = await sts.getCallerIdentity().promise();
  if (promise.Account == impliedAccountId) {
    return true;
  } else {
    console.log(" ==================\n\ = Wrong account! =\n ==================");
    return false;
