...
exports.parseArguments = function(){
  var argv = require('yargs')
      ...
      .check(function(argv, options){
        if((argv.stack) || (argv.project && argv.environment)){
          return true;
        } else {
  ...
  // Decode the stack name, if supplied
  if(argv.stack){
    var stack = _.split(argv.stack, '-');
    envName = _.last(stack).substring(1);
    projectName = _.initial(stack).join('-')
  }
